// Package handlers provides core API handler functionality for the VoltGate server.
...
import (
	...
	coreexecutor "github.com/voltgate/voltgate/v6/sdk/voltgate/executor"
...
const (
	...
	pinnedAuthHeaderName            = "X-CPA-Pinned-Auth-ID"
	selectedAuthHeaderName          = "X-CPA-Selected-Auth-ID"
)
...
func requestExecutionMetadata(ctx context.Context) map[string]any {
	...
	if ctx != nil {
		if value, ok := ctx.Value("gin").(*gin.Context); ok && value != nil && value.Request != nil {
			ginCtx = value
	...
	if pinnedAuthID := pinnedAuthIDFromContext(ctx); pinnedAuthID != "" {
		...
		if pinnedAuthID := strings.TrimSpace(ginCtx.GetHeader(pinnedAuthHeaderName)); pinnedAuthID != "" {
			meta[coreexecutor.PinnedAuthMetadataKey] = pinnedAuthID
		}
	}
	selectedCallback := selectedAuthIDCallbackFromContext(ctx)
	if ginCtx != nil || selectedCallback != nil {
		meta[coreexecutor.SelectedAuthCallbackMetadataKey] = func(authID string) {
			...
			if ginCtx != nil {
				ginCtx.Set(coreexecutor.SelectedAuthMetadataKey, authID)
				ginCtx.Header(selectedAuthHeaderName, authID)
			}
