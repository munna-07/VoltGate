...
func (s *FileTokenStore) Save(ctx context.Context, auth *voltgateauth.Auth) (string, error) {
	if auth == nil {
...
func (s *FileTokenStore) List(ctx context.Context) ([]*voltgateauth.Auth, error) {
	dir := s.baseDirSnapshot()
	if dir == "" {
		return nil, fmt.Errorf("auth filestore: directory not configured")
	}
	entries := make([]*voltgateauth.Auth, 0)
...
func (s *FileTokenStore) readAuthFile(path, baseDir string) (*voltgateauth.Auth, error) {
	...
	if disabled {
		status = voltgateauth.StatusDisabled
	}
...
func (s *FileTokenStore) resolveAuthPath(auth *voltgateauth.Auth) (string, error) {
	if auth == nil {
		return "", fmt.Errorf("auth filestore: auth is nil")
	}
	if auth.Attributes != nil {
