// Package voltgate provides the core service implementation for the VoltGate.
...
import (
	...
	"github.com/voltgate/voltgate/v6/internal/api"
	"github.com/voltgate/voltgate/v6/internal/registry"
	"github.com/voltgate/voltgate/v6/internal/runtime/executor"
	_ "github.com/voltgate/voltgate/v6/internal/usage"
	"github.com/voltgate/voltgate/v6/internal/watcher"
	...
	"github.com/voltgate/voltgate/v6/sdk/voltgate/usage"
	"github.com/voltgate/voltgate/v6/sdk/config"
...
func (s *Service) Run(ctx context.Context) error {
	if s == nil {
		return fmt.Errorf("voltgate: service is nil")
	}
	if ctx == nil {
		ctx = context.Background()
	}
	...
	if err != nil {
		return fmt.Errorf("voltgate: failed to create watcher: %w", err)
	}
	...
	if err = watcherWrapper.Start(watcherCtx); err != nil {
		return fmt.Errorf("voltgate: failed to start watcher: %w", err)
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			if mkErr := os.MkdirAll(s.cfg.AuthDir, 0o755); mkErr != nil {
				return fmt.Errorf("voltgate: failed to create auth directory %s: %w", s.cfg.AuthDir, mkErr)
			}
			log.Infof("created missing auth directory: %s", s.cfg.AuthDir)
			return nil
		}
		return fmt.Errorf("voltgate: error checking auth directory %s: %w", s.cfg.AuthDir, err)
	}
	if !info.IsDir() {
		return fmt.Errorf("voltgate: auth path exists but is not a directory: %s", s.cfg.AuthDir)
	}
	return nil
}
