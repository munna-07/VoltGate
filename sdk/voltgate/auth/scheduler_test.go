...
func (schedulerTestExecutor) Execute(ctx context.Context, auth *Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	return voltgateexecutor.Response{}, nil
}

func (schedulerTestExecutor) ExecuteStream(ctx context.Context, auth *Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (*voltgateexecutor.StreamResult, error) {
	return nil, nil
}
...
func (schedulerTestExecutor) CountTokens(ctx context.Context, auth *Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	return voltgateexecutor.Response{}, nil
}
...
func (s *trackingSelector) Pick(ctx context.Context, provider, model string, opts voltgateexecutor.Options, auths []*Auth) (*Auth, error) {
	s.calls++
...
func TestSchedulerPick_RoundRobinHighestPriority(t *testing.T) {
	...
	for index, wantID := range want {
		got, errPick := scheduler.pickSingle(context.Background(), "gemini", "", voltgateexecutor.Options{}, nil)
		if errPick != nil {
...
func TestSchedulerPick_PromotesExpiredCooldownBeforePick(t *testing.T) {
	...
	got, errPick := scheduler.pickSingle(context.Background(), "gemini", model, voltgateexecutor.Options{}, nil)
	if errPick != nil {
...
func TestSchedulerPick_GeminiVirtualParentUsesTwoLevelRotation(t *testing.T) {
	...
	for index := range wantIDs {
		got, errPick := scheduler.pickSingle(context.Background(), "gemini-cli", "gemini-2.5-pro", voltgateexecutor.Options{}, nil)
		if errPick != nil {
			t.Fatalf("pickSingle() #%d error = %v", index, errPick)
		}
		if got == nil {
...
func TestSchedulerPick_CodexWebsocketPrefersWebsocketEnabledSubset(t *testing.T) {
	...
	ctx := voltgateexecutor.WithDownstreamWebsocket(context.Background())
	want := []string{"codex-ws-a", "codex-ws-b", "codex-ws-a"}
	for index, wantID := range want {
		got, errPick := scheduler.pickSingle(ctx, "codex", "", voltgateexecutor.Options{}, nil)
		if errPick != nil {
			t.Fatalf("pickSingle() #%d error = %v", index, errPick)
		}
		if got == nil {
...
func TestSchedulerPick_MixedProvidersUsesProviderRotationOverReadyCandidates(t *testing.T) {
	...
	for index := range wantProviders {
		got, provider, errPick := scheduler.pickMixed(context.Background(), []string{"gemini", "claude"}, "", voltgateexecutor.Options{}, nil)
		if errPick != nil {
...
func TestSchedulerPick_MixedProvidersPrefersHighestPriorityTier(t *testing.T) {
	...
	for index := range wantProviders {
		got, provider, errPick := scheduler.pickMixed(context.Background(), providers, model, voltgateexecutor.Options{}, nil)
		if errPick != nil {
			t.Fatalf("pickMixed() #%d error = %v", index, errPick)
		}
		if got == nil {
...
func TestManager_PickNextMixed_UsesProviderRotationBeforeCredentialRotation(t *testing.T) {
	...
	wantIDs := []string{"gemini-a", "claude-a", "gemini-b", "claude-a"}
	for index := range wantProviders {
		got, _, provider, errPick := manager.pickNextMixed(context.Background(), []string{"gemini", "claude"}, "", voltgateexecutor.Options{}, map[string]struct{}{})
		if errPick != nil {
...
func TestManagerCustomSelector_FallsBackToLegacyPath(t *testing.T) {
	...
	got, _, errPick := manager.pickNext(context.Background(), "gemini", "", voltgateexecutor.Options{}, map[string]struct{}{})
	if errPick != nil {
...
func TestManager_SchedulerTracksRegisterAndUpdate(t *testing.T) {
	...
	got, errPick := manager.scheduler.pickSingle(context.Background(), "gemini", "", voltgateexecutor.Options{}, nil)
	if errPick != nil {
...
func TestManager_PickNextMixed_UsesSchedulerRotation(t *testing.T) {
	...
	wantIDs := []string{"gemini-a", "claude-a", "gemini-b", "claude-a"}
	for index := range wantProviders {
		got, _, provider, errPick := manager.pickNextMixed(context.Background(), []string{"gemini", "claude"}, "", voltgateexecutor.Options{}, nil)
		if errPick != nil {
			t.Fatalf("pickNextMixed() #%d error = %v", index, errPick)
		}
		if got == nil {
...
func TestManager_SchedulerTracksMarkResultCooldownAndRecovery(t *testing.T) {
	...
	got, errPick := manager.scheduler.pickSingle(context.Background(), "gemini", "test-model", voltgateexecutor.Options{}, nil)
	if errPick != nil {
