...
func (s *authScheduler) pickSingle(ctx context.Context, provider, model string, opts voltgateexecutor.Options, tried map[string]struct{}) (*Auth, error) {
	if s == nil {
	...
	preferWebsocket := voltgateexecutor.DownstreamWebsocket(ctx) && providerKey == "codex" && pinnedAuthID == ""
...
func (s *authScheduler) pickMixed(ctx context.Context, providers []string, model string, opts voltgateexecutor.Options, tried map[string]struct{}) (*Auth, string, error) {
	if s == nil {
		return nil, "", &Error{Code: "auth_not_found", Message: "no auth available"}
	}
