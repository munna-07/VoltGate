...
type openAICompatPoolExecutor struct {
	...
	streamPayloads    map[string][]voltgateexecutor.StreamChunk
}

func (e *openAICompatPoolExecutor) Identifier() string { return e.id }

func (e *openAICompatPoolExecutor) Execute(ctx context.Context, auth *Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	_ = ctx
	_ = auth
	_ = opts
	e.mu.Lock()
	...
	return voltgateexecutor.Response{Payload: []byte(req.Model)}, nil
}

func (e *openAICompatPoolExecutor) ExecuteStream(ctx context.Context, auth *Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (*voltgateexecutor.StreamResult, error) {
	_ = ctx
	_ = auth
	_ = opts
	e.mu.Lock()
	...
	chunks := append([]voltgateexecutor.StreamChunk(nil), payloadChunks...)
	e.mu.Unlock()
	ch := make(chan voltgateexecutor.StreamChunk, max(1, len(chunks)))
	if err != nil {
		ch <- voltgateexecutor.StreamChunk{Err: err}
		close(ch)
		return &voltgateexecutor.StreamResult{Headers: http.Header{"X-Model": {req.Model}}, Chunks: ch}, nil
	}
	if !hasCustomChunks {
		ch <- voltgateexecutor.StreamChunk{Payload: []byte(req.Model)}
	} else {
...
func (e *openAICompatPoolExecutor) CountTokens(ctx context.Context, auth *Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	_ = ctx
	_ = auth
	_ = opts
	e.mu.Lock()
...
func TestManagerExecuteCount_OpenAICompatAliasPoolStopsOnInvalidRequest(t *testing.T) {
	...
	_, err := m.ExecuteCount(context.Background(), []string{"pool"}, voltgateexecutor.Request{Model: alias}, voltgateexecutor.Options{})
...
func TestManagerExecute_OpenAICompatAliasPoolRotatesWithinAuth(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		resp, err := m.Execute(context.Background(), []string{"pool"}, voltgateexecutor.Request{Model: alias}, voltgateexecutor.Options{})
		if err != nil {
...
func TestManagerExecute_OpenAICompatAliasPoolStopsOnBadRequest(t *testing.T) {
	...
	_, err := m.Execute(context.Background(), []string{"pool"}, voltgateexecutor.Request{Model: alias}, voltgateexecutor.Options{})
	if err == nil || err.Error() != invalidErr.Error() {
...
func TestManagerExecuteStream_OpenAICompatAliasPoolRetriesOnEmptyBootstrap(t *testing.T) {
	...
	streamResult, err := m.ExecuteStream(context.Background(), []string{"pool"}, voltgateexecutor.Request{Model: alias}, voltgateexecutor.Options{})
	if err != nil {
...
func TestManagerExecuteStream_OpenAICompatAliasPoolStopsOnInvalidRequest(t *testing.T) {
	...
	_, err := m.ExecuteStream(context.Background(), []string{"pool"}, voltgateexecutor.Request{Model: alias}, voltgateexecutor.Options{})
	if err == nil || err.Error() != invalidErr.Error() {
...
func TestManagerExecuteCount_OpenAICompatAliasPoolRotatesWithinAuth(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		resp, err := m.ExecuteCount(context.Background(), []string{"pool"}, voltgateexecutor.Request{Model: alias}, voltgateexecutor.Options{})
		if err != nil {
