...
func TestFillFirstSelectorPick_Deterministic(t *testing.T) {
	...
	got, err := selector.Pick(context.Background(), "gemini", "", voltgateexecutor.Options{}, auths)
	if err != nil {
...
func TestRoundRobinSelectorPick_PriorityBuckets(t *testing.T) {
	...
	want := []string{"a", "b", "a", "b"}
	for i, id := range want {
		got, err := selector.Pick(context.Background(), "mixed", "", voltgateexecutor.Options{}, auths)
		if err != nil {
			t.Fatalf("Pick() #%d error = %v", i, err)
		}
		if got == nil {
...
func TestFillFirstSelectorPick_PriorityFallbackCooldown(t *testing.T) {
	...
	got, err := selector.Pick(context.Background(), "mixed", model, voltgateexecutor.Options{}, []*Auth{high, low})
	if err != nil {
		t.Fatalf("Pick() error = %v", err)
	}
	if got == nil {
...
func TestSelectorPick_AllCooldownReturnsModelCooldownError(t *testing.T) {
	...
	t.Run("mixed provider redacts provider field", func(t *testing.T) {
		...
		selector := &FillFirstSelector{}
		_, err := selector.Pick(context.Background(), "mixed", model, voltgateexecutor.Options{}, auths)
		if err == nil {
	...
	t.Run("non-mixed provider includes provider field", func(t *testing.T) {
		...
		selector := &FillFirstSelector{}
		_, err := selector.Pick(context.Background(), "gemini", model, voltgateexecutor.Options{}, auths)
		if err == nil {
			t.Fatalf("Pick() error = nil")
		}
...
func TestFillFirstSelectorPick_ThinkingSuffixFallsBackToBaseModelState(t *testing.T) {
	...
	got, err := selector.Pick(context.Background(), "mixed", requestedModel, voltgateexecutor.Options{}, []*Auth{high, low})
	if err != nil {
		t.Fatalf("Pick() error = %v", err)
	}
	if got == nil {
...
func TestRoundRobinSelectorPick_ThinkingSuffixSharesCursor(t *testing.T) {
	...
	first, err := selector.Pick(context.Background(), "gemini", "test-model(high)", voltgateexecutor.Options{}, auths)
	if err != nil {
		t.Fatalf("Pick() first error = %v", err)
	}
	second, err := selector.Pick(context.Background(), "gemini", "test-model(low)", voltgateexecutor.Options{}, auths)
	if err != nil {
...
func TestRoundRobinSelectorPick_CursorKeyCap(t *testing.T) {
	...
	_, _ = selector.Pick(context.Background(), "gemini", "m1", voltgateexecutor.Options{}, auths)
	_, _ = selector.Pick(context.Background(), "gemini", "m2", voltgateexecutor.Options{}, auths)
	_, _ = selector.Pick(context.Background(), "gemini", "m3", voltgateexecutor.Options{}, auths)
...
func TestRoundRobinSelectorPick_GeminiCLICredentialGrouping(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		got, err := selector.Pick(context.Background(), "gemini-cli", "gemini-2.5-pro", voltgateexecutor.Options{}, auths)
		if err != nil {
			t.Fatalf("Pick() #%d error = %v", i, err)
		}
		if got == nil {
...
func TestRoundRobinSelectorPick_MixedVirtualAndNonVirtualFallsBackToFlat(t *testing.T) {
	...
	for i, expectedID := range want {
		got, err := selector.Pick(context.Background(), "gemini-cli", "", voltgateexecutor.Options{}, auths)
		if err != nil {
			t.Fatalf("Pick() #%d error = %v", i, err)
		}
		if got == nil {
