...
func (e *credentialRetryLimitExecutor) Execute(context.Context, *Auth, voltgateexecutor.Request, voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	e.recordCall()
	return voltgateexecutor.Response{}, &Error{HTTPStatus: 500, Message: "boom"}
}

func (e *credentialRetryLimitExecutor) ExecuteStream(context.Context, *Auth, voltgateexecutor.Request, voltgateexecutor.Options) (*voltgateexecutor.StreamResult, error) {
	e.recordCall()
...
func (e *credentialRetryLimitExecutor) CountTokens(context.Context, *Auth, voltgateexecutor.Request, voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	e.recordCall()
	return voltgateexecutor.Response{}, &Error{HTTPStatus: 500, Message: "boom"}
}
...
func TestManager_MaxRetryCredentials_LimitsCrossCredentialRetries(t *testing.T) {
	request := voltgateexecutor.Request{Model: "test-model"}
	testCases := []struct {
		name   string
	...
	}{
		{
			...
			invoke: func(m *Manager) error {
				_, errExecute := m.Execute(context.Background(), []string{"claude"}, request, voltgateexecutor.Options{})
				return errExecute
			},
		},
		{
			name: "execute_count",
			invoke: func(m *Manager) error {
				_, errExecute := m.ExecuteCount(context.Background(), []string{"claude"}, request, voltgateexecutor.Options{})
				return errExecute
			},
		},
		{
			name: "execute_stream",
			invoke: func(m *Manager) error {
				_, errExecute := m.ExecuteStream(context.Background(), []string{"claude"}, request, voltgateexecutor.Options{})
				return errExecute
			},
		},
	}
