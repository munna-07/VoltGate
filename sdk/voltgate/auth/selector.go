...
func preferCodexWebsocketAuths(ctx context.Context, provider string, available []*Auth) []*Auth {
	...
	if !voltgateexecutor.DownstreamWebsocket(ctx) {
		return available
	}
...
func (s *RoundRobinSelector) Pick(ctx context.Context, provider, model string, opts voltgateexecutor.Options, auths []*Auth) (*Auth, error) {
	_ = opts
	now := time.Now()
...
func (s *FillFirstSelector) Pick(ctx context.Context, provider, model string, opts voltgateexecutor.Options, auths []*Auth) (*Auth, error) {
	_ = opts
	now := time.Now()
	available, err := getAvailableAuths(auths, provider, model, now)
	if err != nil {
