...
import (
	...
	internalconfig "github.com/voltgate/voltgate/v6/internal/config"
	"github.com/voltgate/voltgate/v6/internal/logging"
	"github.com/voltgate/voltgate/v6/internal/registry"
	"github.com/voltgate/voltgate/v6/internal/thinking"
	"github.com/voltgate/voltgate/v6/internal/util"
...
type ProviderExecutor interface {
	...
	Execute(ctx context.Context, auth *Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error)
	...
	ExecuteStream(ctx context.Context, auth *Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (*voltgateexecutor.StreamResult, error)
	// Refresh attempts to refresh provider credentials and returns the updated auth state.
	Refresh(ctx context.Context, auth *Auth) (*Auth, error)
	// CountTokens returns the token count for the given request.
	CountTokens(ctx context.Context, auth *Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error)
...
type Selector interface {
	Pick(ctx context.Context, provider, model string, opts voltgateexecutor.Options, auths []*Auth) (*Auth, error)
}
...
func discardStreamChunks(ch <-chan voltgateexecutor.StreamChunk) {
	if ch == nil {
		return
	}
	go func() {
...
func readStreamBootstrap(ctx context.Context, ch <-chan voltgateexecutor.StreamChunk) ([]voltgateexecutor.StreamChunk, bool, error) {
	if ch == nil {
		return nil, true, nil
	}
	buffered := make([]voltgateexecutor.StreamChunk, 0, 1)
	for {
		var (
			chunk voltgateexecutor.StreamChunk
			ok    bool
		)
		if ctx != nil {
			select {
...
func (m *Manager) wrapStreamResult(ctx context.Context, auth *Auth, provider, routeModel string, headers http.Header, buffered []voltgateexecutor.StreamChunk, remaining <-chan voltgateexecutor.StreamChunk) *voltgateexecutor.StreamResult {
	out := make(chan voltgateexecutor.StreamChunk)
	go func() {
		defer close(out)
		var failed bool
		forward := true
		emit := func(chunk voltgateexecutor.StreamChunk) bool {
			if chunk.Err != nil && !failed {
				failed = true
				rerr := &Error{Message: chunk.Err.Error()}
				if se, ok := errors.AsType[voltgateexecutor.StatusError](chunk.Err); ok && se != nil {
	...
	return &voltgateexecutor.StreamResult{Headers: headers, Chunks: out}
}

func (m *Manager) executeStreamWithModelPool(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req voltgateexecutor.Request, opts voltgateexecutor.Options, routeModel string) (*voltgateexecutor.StreamResult, error) {
	if executor == nil {
	...
	for idx, execModel := range execModels {
		...
		if errStream != nil {
			...
			if se, ok := errors.AsType[voltgateexecutor.StatusError](errStream); ok && se != nil {
				rerr.HTTPStatus = se.StatusCode()
			}
		...
		if bootstrapErr != nil {
			...
			if isRequestInvalidError(bootstrapErr) {
				...
				if se, ok := errors.AsType[voltgateexecutor.StatusError](bootstrapErr); ok && se != nil {
					rerr.HTTPStatus = se.StatusCode()
				}
				result := Result{AuthID: auth.ID, Provider: provider, Model: routeModel, Success: false, Error: rerr}
			...
			errCh := make(chan voltgateexecutor.StreamChunk, 1)
			errCh <- voltgateexecutor.StreamChunk{Err: bootstrapErr}
			close(errCh)
		...
		if closed && len(buffered) == 0 {
			...
			errCh := make(chan voltgateexecutor.StreamChunk, 1)
			errCh <- voltgateexecutor.StreamChunk{Err: emptyErr}
			close(errCh)
			return m.wrapStreamResult(ctx, auth.Clone(), provider, routeModel, streamResult.Headers, nil, errCh), nil
		}
		...
		if closed {
			closedCh := make(chan voltgateexecutor.StreamChunk)
			close(closedCh)
			remaining = closedCh
		}
...
func (m *Manager) Execute(ctx context.Context, providers []string, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	normalized := m.normalizeProviders(providers)
	if len(normalized) == 0 {
		return voltgateexecutor.Response{}, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	...
	for attempt := 0; ; attempt++ {
		...
		if errWait := waitForCooldown(ctx, wait); errWait != nil {
			return voltgateexecutor.Response{}, errWait
		}
	}
	if lastErr != nil {
		return voltgateexecutor.Response{}, lastErr
	}
	return voltgateexecutor.Response{}, &Error{Code: "auth_not_found", Message: "no auth available"}
}

// ExecuteCount performs a non-streaming execution using the configured selector and executor.
// It supports multiple providers for the same model and round-robins the starting provider per model.
func (m *Manager) ExecuteCount(ctx context.Context, providers []string, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	normalized := m.normalizeProviders(providers)
	if len(normalized) == 0 {
		return voltgateexecutor.Response{}, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
...
// It supports multiple providers for the same model and round-robins the starting provider per model.
func (m *Manager) ExecuteStream(ctx context.Context, providers []string, req voltgateexecutor.Request, opts voltgateexecutor.Options) (*voltgateexecutor.StreamResult, error) {
	normalized := m.normalizeProviders(providers)
	if len(normalized) == 0 {
...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req voltgateexecutor.Request, opts voltgateexecutor.Options, maxRetryCredentials int) (voltgateexecutor.Response, error) {
	if len(providers) == 0 {
		return voltgateexecutor.Response{}, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	...
	for {
		...
		if errPick != nil {
			...
			return voltgateexecutor.Response{}, errPick
		}
		...
		if rt := m.roundTripperFor(auth); rt != nil {
			...
			execCtx = context.WithValue(execCtx, "voltgate.roundtripper", rt)
		}
		...
		for _, upstreamModel := range models {
			...
			if errExec != nil {
				if errCtx := execCtx.Err(); errCtx != nil {
					return voltgateexecutor.Response{}, errCtx
				}
				result.Error = &Error{Message: errExec.Error()}
				if se, ok := errors.AsType[voltgateexecutor.StatusError](errExec); ok && se != nil {
				...
				if isRequestInvalidError(errExec) {
					return voltgateexecutor.Response{}, errExec
				}
				authErr = errExec
				continue
			}
		...
		if authErr != nil {
			if isRequestInvalidError(authErr) {
				return voltgateexecutor.Response{}, authErr
			}
			lastErr = authErr
			continue
		}
...
func (m *Manager) executeCountMixedOnce(ctx context.Context, providers []string, req voltgateexecutor.Request, opts voltgateexecutor.Options, maxRetryCredentials int) (voltgateexecutor.Response, error) {
	if len(providers) == 0 {
		return voltgateexecutor.Response{}, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	routeModel := req.Model
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req voltgateexecutor.Request, opts voltgateexecutor.Options, maxRetryCredentials int) (*voltgateexecutor.StreamResult, error) {
	if len(providers) == 0 {
		return nil, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	routeModel := req.Model
...
func ensureRequestedModelMetadata(opts voltgateexecutor.Options, requestedModel string) voltgateexecutor.Options {
	requestedModel = strings.TrimSpace(requestedModel)
	if requestedModel == "" {
		return opts
	}
	...
	if len(opts.Metadata) == 0 {
		opts.Metadata = map[string]any{voltgateexecutor.RequestedModelMetadataKey: requestedModel}
		return opts
	}
	...
	meta[voltgateexecutor.RequestedModelMetadataKey] = requestedModel
	opts.Metadata = meta
	return opts
}
...
func hasRequestedModelMetadata(meta map[string]any) bool {
	...
	raw, ok := meta[voltgateexecutor.RequestedModelMetadataKey]
	if !ok || raw == nil {
		return false
	}
	switch v := raw.(type) {
...
func pinnedAuthIDFromMetadata(meta map[string]any) string {
	...
	raw, ok := meta[voltgateexecutor.PinnedAuthMetadataKey]
	if !ok || raw == nil {
		return ""
	}
	switch val := raw.(type) {
...
func publishSelectedAuthMetadata(meta map[string]any, authID string) {
	...
	meta[voltgateexecutor.SelectedAuthMetadataKey] = authID
	if callback, ok := meta[voltgateexecutor.SelectedAuthCallbackMetadataKey].(func(string)); ok && callback != nil {
		callback(authID)
	}
}
...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
	...
	suspendReason := ""
	resumeModels := make([]string, 0, 1)
	suspendModels := make([]string, 0, 1)
	clearModelQuotaModels := make([]string, 0, 1)
	setModelQuotaModels := make([]string, 0, 1)
	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		...
		if result.Success {
			if result.Model != "" {
				targetModels := sharedQuotaModelsForAuth(auth, result.Model)
				if len(targetModels) == 0 {
					targetModels = []string{result.Model}
				}
				for i := range targetModels {
					resetModelState(ensureModelState(auth, targetModels[i]), now)
				}
				...
				auth.UpdatedAt = now
				resumeModels = appendUniqueModels(resumeModels, targetModels...)
				clearModelQuotaModels = appendUniqueModels(clearModelQuotaModels, targetModels...)
			} else {
			...
			if result.Model != "" {
				...
				case 401:
					...
					suspendModels = appendUniqueModels(suspendModels, result.Model)
				case 402, 403:
					next := now.Add(30 * time.Minute)
					state.NextRetryAfter = next
				...
				case 429:
					...
					for i := range targetModels {
						...
						targetState := ensureModelState(auth, targetModel)
						targetState.Unavailable = true
						targetState.Status = StatusError
						targetState.StatusMessage = state.StatusMessage
						targetState.NextRetryAfter = next
						targetState.LastError = cloneError(result.Error)
						targetState.Quota = QuotaState{
							Exceeded:      true,
							Reason:        "quota",
							NextRecoverAt: next,
							BackoffLevel:  backoffLevel,
					...
					suspendReason = "quota"
					suspendModels = appendUniqueModels(suspendModels, targetModels...)
					setModelQuotaModels = appendUniqueModels(setModelQuotaModels, targetModels...)
	...
	for i := range clearModelQuotaModels {
		registry.GetGlobalRegistry().ClearModelQuotaExceeded(result.AuthID, clearModelQuotaModels[i])
	}
	for i := range setModelQuotaModels {
		registry.GetGlobalRegistry().SetModelQuotaExceeded(result.AuthID, setModelQuotaModels[i])
	}
	for i := range resumeModels {
		registry.GetGlobalRegistry().ResumeClientModel(result.AuthID, resumeModels[i])
	}
	for i := range suspendModels {
		registry.GetGlobalRegistry().SuspendClientModel(result.AuthID, suspendModels[i], suspendReason)
	}
...
func (m *Manager) pickNextLegacy(ctx context.Context, provider, model string, opts voltgateexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
...
func (m *Manager) pickNext(ctx context.Context, provider, model string, opts voltgateexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
...
func (m *Manager) pickNextMixedLegacy(ctx context.Context, providers []string, model string, opts voltgateexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	pinnedAuthID := pinnedAuthIDFromMetadata(opts.Metadata)
...
func (m *Manager) pickNextMixed(ctx context.Context, providers []string, model string, opts voltgateexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	if !m.useSchedulerFastPath() {
