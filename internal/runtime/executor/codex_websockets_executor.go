...
import (
	...
	"github.com/voltgate/voltgate/v6/internal/misc"
	...
	"github.com/voltgate/voltgate/v6/sdk/proxyutil"
...
func (e *CodexWebsocketsExecutor) Execute(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (resp voltgateexecutor.Response, err error) {
	if ctx == nil {
	...
	for {
		...
		if eventType == "response.completed" {
			...
			resp = voltgateexecutor.Response{Payload: []byte(out)}
			return resp, nil
		}
	}
}

func (e *CodexWebsocketsExecutor) ExecuteStream(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (_ *voltgateexecutor.StreamResult, err error) {
	...
	go func() {
		...
		send := func(chunk voltgateexecutor.StreamChunk) bool {
			if ctx == nil {
				out <- chunk
				return true
			}
		...
		for {
			if ctx != nil && ctx.Err() != nil {
				...
				_ = send(voltgateexecutor.StreamChunk{Err: ctx.Err()})
				return
			}
			msgType, payload, errRead := readCodexWebsocketMessage(ctx, sess, conn, readCh)
			if errRead != nil {
				...
				_ = send(voltgateexecutor.StreamChunk{Err: errRead})
				return
			}
			if msgType != websocket.TextMessage {
				if msgType == websocket.BinaryMessage {
					...
					_ = send(voltgateexecutor.StreamChunk{Err: err})
					return
				}
				continue
			}
			...
			if wsErr, ok := parseCodexWebsocketError(payload); ok {
				...
				_ = send(voltgateexecutor.StreamChunk{Err: wsErr})
				return
			}
			...
			for i := range chunks {
				if !send(voltgateexecutor.StreamChunk{Payload: []byte(chunks[i])}) {
					terminateReason = "context_done"
					terminateErr = ctx.Err()
					return
				}
	...
	return &voltgateexecutor.StreamResult{Headers: upstreamHeaders, Chunks: out}, nil
}

func (e *CodexWebsocketsExecutor) dialCodexWebsocket(ctx context.Context, auth *voltgateauth.Auth, wsURL string, headers http.Header) (*websocket.Conn, *http.Response, error) {
...
func newProxyAwareWebsocketDialer(cfg *config.Config, auth *voltgateauth.Auth) *websocket.Dialer {
...
func applyCodexPromptCacheHeaders(from sdktranslator.Format, req voltgateexecutor.Request, rawJSON []byte) ([]byte, http.Header) {
	headers := http.Header{}
	if len(rawJSON) == 0 {
...
func applyCodexWebsocketHeaders(ctx context.Context, headers http.Header, auth *voltgateauth.Auth, token string, cfg *config.Config) http.Header {
	if headers == nil {
		headers = http.Header{}
	}
...
func codexHeaderDefaults(cfg *config.Config, auth *voltgateauth.Auth) (string, string) {
	if cfg == nil || auth == nil {
		return "", ""
	}
...
func executionSessionIDFromOptions(opts voltgateexecutor.Options) string {
	if len(opts.Metadata) == 0 {
		return ""
	}
	raw, ok := opts.Metadata[voltgateexecutor.ExecutionSessionMetadataKey]
	if !ok || raw == nil {
		return ""
	}
	switch v := raw.(type) {
...
func (e *CodexWebsocketsExecutor) ensureUpstreamConn(ctx context.Context, auth *voltgateauth.Auth, sess *codexWebsocketSession, authID string, wsURL string, headers http.Header) (*websocket.Conn, *http.Response, error) {
	if sess == nil {
...
func (e *CodexWebsocketsExecutor) CloseExecutionSession(sessionID string) {
	...
	if sessionID == voltgateauth.CloseAllExecutionSessionsID {
...
func (e *CodexAutoExecutor) PrepareRequest(req *http.Request, auth *voltgateauth.Auth) error {
...
func (e *CodexAutoExecutor) HttpRequest(ctx context.Context, auth *voltgateauth.Auth, req *http.Request) (*http.Response, error) {
	if e == nil || e.httpExec == nil {
...
func (e *CodexAutoExecutor) Execute(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	if e == nil || e.httpExec == nil || e.wsExec == nil {
		return voltgateexecutor.Response{}, fmt.Errorf("codex auto executor: executor is nil")
	}
	if voltgateexecutor.DownstreamWebsocket(ctx) && codexWebsocketsEnabled(auth) {
		return e.wsExec.Execute(ctx, auth, req, opts)
	}
	return e.httpExec.Execute(ctx, auth, req, opts)
}

func (e *CodexAutoExecutor) ExecuteStream(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (*voltgateexecutor.StreamResult, error) {
	if e == nil || e.httpExec == nil || e.wsExec == nil {
...
func (e *CodexAutoExecutor) Refresh(ctx context.Context, auth *voltgateauth.Auth) (*voltgateauth.Auth, error) {
	if e == nil || e.httpExec == nil {
		return nil, fmt.Errorf("codex auto executor: http executor is nil")
	}
...
func (e *CodexAutoExecutor) CountTokens(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	if e == nil || e.httpExec == nil {
		return voltgateexecutor.Response{}, fmt.Errorf("codex auto executor: http executor is nil")
	}
...
func codexWebsocketsEnabled(auth *voltgateauth.Auth) bool {
	if auth == nil {
		return false
	}
