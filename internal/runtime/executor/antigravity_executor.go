...
import (
	...
	antigravityauth "github.com/voltgate/voltgate/v6/internal/auth/antigravity"
...
func newAntigravityHTTPClient(ctx context.Context, cfg *config.Config, auth *voltgateauth.Auth, timeout time.Duration) *http.Client {
...
func (e *AntigravityExecutor) PrepareRequest(req *http.Request, auth *voltgateauth.Auth) error {
	if req == nil {
		return nil
	}
...
func (e *AntigravityExecutor) HttpRequest(ctx context.Context, auth *voltgateauth.Auth, req *http.Request) (*http.Response, error) {
	if req == nil {
...
func (e *AntigravityExecutor) Execute(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (resp voltgateexecutor.Response, err error) {
...
attemptLoop:
	for attempt := 0; attempt < attempts; attempt++ {
		...
		for idx, baseURL := range baseURLs {
			...
			resp = voltgateexecutor.Response{Payload: []byte(converted), Headers: httpResp.Header.Clone()}
...
func (e *AntigravityExecutor) executeClaudeNonStream(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (resp voltgateexecutor.Response, err error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
		...
		for idx, baseURL := range baseURLs {
			...
			go func(resp *http.Response) {
				...
				for scanner.Scan() {
					...
					out <- voltgateexecutor.StreamChunk{Payload: payload}
				}
			...
			resp = voltgateexecutor.Response{Payload: e.convertStreamToNonStream(buffer.Bytes())}
...
func (e *AntigravityExecutor) ExecuteStream(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (_ *voltgateexecutor.StreamResult, err error) {
	if opts.Alt == "responses/compact" {
		...
		for idx, baseURL := range baseURLs {
			...
			go func(resp *http.Response) {
				...
				for scanner.Scan() {
					...
					for i := range chunks {
						out <- voltgateexecutor.StreamChunk{Payload: []byte(chunks[i])}
					}
				}
				tail := sdktranslator.TranslateStream(ctx, to, from, req.Model, opts.OriginalRequest, translated, []byte("[DONE]"), &param)
				for i := range tail {
					out <- voltgateexecutor.StreamChunk{Payload: []byte(tail[i])}
				}
				if errScan := scanner.Err(); errScan != nil {
					recordAPIResponseError(ctx, e.cfg, errScan)
					reporter.publishFailure(ctx)
...
func (e *AntigravityExecutor) Refresh(ctx context.Context, auth *voltgateauth.Auth) (*voltgateauth.Auth, error) {
	if auth == nil {
		return auth, nil
	}
...
func (e *AntigravityExecutor) CountTokens(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName

	token, updatedAuth, errToken := e.ensureAccessToken(ctx, auth)
	if errToken != nil {
		return voltgateexecutor.Response{}, errToken
	}
	if updatedAuth != nil {
		auth = updatedAuth
	}
	if strings.TrimSpace(token) == "" {
		return voltgateexecutor.Response{}, statusErr{code: http.StatusUnauthorized, msg: "missing access token"}
	}
	...
	for idx, baseURL := range baseURLs {
		...
		if errReq != nil {
			return voltgateexecutor.Response{}, errReq
		}
		httpReq.Close = true
		...
		if httpResp.StatusCode >= http.StatusOK && httpResp.StatusCode < http.StatusMultipleChoices {
			...
			return voltgateexecutor.Response{Payload: []byte(translated), Headers: httpResp.Header.Clone()}, nil
		}
		...
		return voltgateexecutor.Response{}, sErr
	}
	...
	default:
		return voltgateexecutor.Response{}, statusErr{code: http.StatusServiceUnavailable, msg: "antigravity executor: no base url available"}
	}
}

func (e *AntigravityExecutor) ensureAccessToken(ctx context.Context, auth *voltgateauth.Auth) (string, *voltgateauth.Auth, error) {
	if auth == nil {
	...
	if ctx != nil {
		if rt, ok := ctx.Value("voltgate.roundtripper").(http.RoundTripper); ok && rt != nil {
			refreshCtx = context.WithValue(refreshCtx, "voltgate.roundtripper", rt)
		}
	}
...
func (e *AntigravityExecutor) refreshToken(ctx context.Context, auth *voltgateauth.Auth) (*voltgateauth.Auth, error) {
	if auth == nil {
		return nil, statusErr{code: http.StatusUnauthorized, msg: "missing auth"}
	}
...
func (e *AntigravityExecutor) ensureAntigravityProjectID(ctx context.Context, auth *voltgateauth.Auth, accessToken string) error {
	if auth == nil {
		return nil
	}
...
func (e *AntigravityExecutor) buildRequest(ctx context.Context, auth *voltgateauth.Auth, token, modelName string, payload []byte, stream bool, alt, baseURL string) (*http.Request, error) {
	if token == "" {
...
func buildBaseURL(auth *voltgateauth.Auth) string {
...
func resolveUserAgent(auth *voltgateauth.Auth) string {
	if auth != nil {
...
func antigravityRetryAttempts(auth *voltgateauth.Auth, cfg *config.Config) int {
	retry := 0
	if cfg != nil {
...
func antigravityBaseURLFallbackOrder(auth *voltgateauth.Auth) []string {
...
func resolveCustomAntigravityBaseURL(auth *voltgateauth.Auth) string {
	if auth == nil {
		return ""
	}
	if auth.Attributes != nil {
