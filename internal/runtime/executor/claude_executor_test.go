...
func TestClaudeExecutor_ReusesUserIDAcrossModelsWhenCacheEnabled(t *testing.T) {
	...
	auth := &voltgateauth.Auth{Attributes: map[string]string{
		"api_key":  "key-123",
		"base_url": server.URL,
	}}
	...
	for _, model := range models {
		...
		if _, err := executor.Execute(context.Background(), auth, voltgateexecutor.Request{
			Model:   model,
			Payload: modelPayload,
		}, voltgateexecutor.Options{
...
func TestClaudeExecutor_CountTokens_AppliesCacheControlGuards(t *testing.T) {
	...
	_, err := executor.CountTokens(context.Background(), auth, voltgateexecutor.Request{
		Model:   "claude-3-5-haiku-20241022",
		Payload: payload,
	}, voltgateexecutor.Options{SourceFormat: sdktranslator.FromString("claude")})
	if err != nil {
...
func TestClaudeExecutor_Execute_InvalidGzipErrorBodyReturnsDecodeMessage(t *testing.T) {
	testClaudeExecutorInvalidCompressedErrorBody(t, func(executor *ClaudeExecutor, auth *voltgateauth.Auth, payload []byte) error {
		_, err := executor.Execute(context.Background(), auth, voltgateexecutor.Request{
...
func TestClaudeExecutor_ExecuteStream_InvalidGzipErrorBodyReturnsDecodeMessage(t *testing.T) {
	testClaudeExecutorInvalidCompressedErrorBody(t, func(executor *ClaudeExecutor, auth *voltgateauth.Auth, payload []byte) error {
		_, err := executor.ExecuteStream(context.Background(), auth, voltgateexecutor.Request{
			Model:   "claude-3-5-sonnet-20241022",
			Payload: payload,
		}, voltgateexecutor.Options{SourceFormat: sdktranslator.FromString("claude")})
		return err
...
func testClaudeExecutorInvalidCompressedErrorBody(
	t *testing.T,
	invoke func(executor *ClaudeExecutor, auth *voltgateauth.Auth, payload []byte) error,
) {
	t.Helper()
...
func TestClaudeExecutor_ExecuteStream_SetsIdentityAcceptEncoding(t *testing.T) {
	...
	result, err := executor.ExecuteStream(context.Background(), auth, voltgateexecutor.Request{
		Model:   "claude-3-5-sonnet-20241022",
		Payload: payload,
	}, voltgateexecutor.Options{
		SourceFormat: sdktranslator.FromString("claude"),
