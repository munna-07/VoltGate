...
import (
	...
	claudeauth "github.com/voltgate/voltgate/v6/internal/auth/claude"
...
func (e *ClaudeExecutor) PrepareRequest(req *http.Request, auth *voltgateauth.Auth) error {
	if req == nil {
		return nil
	}
...
func (e *ClaudeExecutor) HttpRequest(ctx context.Context, auth *voltgateauth.Auth, req *http.Request) (*http.Response, error) {
	if req == nil {
...
func (e *ClaudeExecutor) Execute(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (resp voltgateexecutor.Response, err error) {
...
func (e *ClaudeExecutor) ExecuteStream(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (_ *voltgateexecutor.StreamResult, err error) {
	if opts.Alt == "responses/compact" {
	...
	go func() {
		...
		if from == to {
			...
			for scanner.Scan() {
				...
				copy(cloned, line)
				cloned[len(line)] = '\n'
				out <- voltgateexecutor.StreamChunk{Payload: cloned}
			}
...
func (e *ClaudeExecutor) CountTokens(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		if decErr != nil {
			...
			msg := fmt.Sprintf("failed to decode error response body: %v", decErr)
			logWithRequestID(ctx).Warn(msg)
			return voltgateexecutor.Response{}, statusErr{code: resp.StatusCode, msg: msg}
		}
		b, readErr := io.ReadAll(errBody)
		if readErr != nil {
			recordAPIResponseError(ctx, e.cfg, readErr)
		...
		return voltgateexecutor.Response{}, statusErr{code: resp.StatusCode, msg: string(b)}
	}
	...
	return voltgateexecutor.Response{Payload: []byte(out), Headers: resp.Header.Clone()}, nil
}

func (e *ClaudeExecutor) Refresh(ctx context.Context, auth *voltgateauth.Auth) (*voltgateauth.Auth, error) {
...
func applyClaudeHeaders(r *http.Request, auth *voltgateauth.Auth, apiKey string, stream bool, extraBetas []string, cfg *config.Config) {
...
func claudeCreds(a *voltgateauth.Auth) (apiKey, baseURL string) {
	if a == nil {
		return "", ""
	}
	if a.Attributes != nil {
...
func getCloakConfigFromAuth(auth *voltgateauth.Auth) (string, bool, []string, bool) {
...
func resolveClaudeKeyCloakConfig(cfg *config.Config, auth *voltgateauth.Auth) *config.CloakConfig {
	if cfg == nil || auth == nil {
		return nil
	}
...
func applyCloaking(ctx context.Context, cfg *config.Config, auth *voltgateauth.Auth, payload []byte, model string, apiKey string) []byte {
