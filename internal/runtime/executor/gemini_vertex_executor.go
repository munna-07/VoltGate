...
import (
	...
	vertexauth "github.com/voltgate/voltgate/v6/internal/auth/vertex"
...
func (e *GeminiVertexExecutor) PrepareRequest(req *http.Request, auth *voltgateauth.Auth) error {
	if req == nil {
		return nil
	}
...
func (e *GeminiVertexExecutor) HttpRequest(ctx context.Context, auth *voltgateauth.Auth, req *http.Request) (*http.Response, error) {
	if req == nil {
...
func (e *GeminiVertexExecutor) Execute(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (resp voltgateexecutor.Response, err error) {
...
func (e *GeminiVertexExecutor) ExecuteStream(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (*voltgateexecutor.StreamResult, error) {
	if opts.Alt == "responses/compact" {
...
func (e *GeminiVertexExecutor) CountTokens(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	// Try API key authentication first
	apiKey, baseURL := vertexAPICreds(auth)
	...
	if apiKey == "" {
		projectID, location, saJSON, errCreds := vertexCreds(auth)
		if errCreds != nil {
			return voltgateexecutor.Response{}, errCreds
		}
...
func (e *GeminiVertexExecutor) Refresh(_ context.Context, auth *voltgateauth.Auth) (*voltgateauth.Auth, error) {
	return auth, nil
}
...
func (e *GeminiVertexExecutor) executeWithServiceAccount(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options, projectID, location string, saJSON []byte) (resp voltgateexecutor.Response, err error) {
	...
	resp = voltgateexecutor.Response{Payload: []byte(out), Headers: httpResp.Header.Clone()}
	return resp, nil
}

// executeWithAPIKey handles authentication using API key credentials.
func (e *GeminiVertexExecutor) executeWithAPIKey(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options, apiKey, baseURL string) (resp voltgateexecutor.Response, err error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
...
func (e *GeminiVertexExecutor) executeStreamWithServiceAccount(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options, projectID, location string, saJSON []byte) (_ *voltgateexecutor.StreamResult, err error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
	...
	go func() {
		...
		for scanner.Scan() {
			...
			for i := range lines {
				out <- voltgateexecutor.StreamChunk{Payload: []byte(lines[i])}
			}
		}
		...
		if errScan := scanner.Err(); errScan != nil {
			...
			out <- voltgateexecutor.StreamChunk{Err: errScan}
		}
	}()
	return &voltgateexecutor.StreamResult{Headers: httpResp.Header.Clone(), Chunks: out}, nil
}

// executeStreamWithAPIKey handles streaming authentication using API key credentials.
func (e *GeminiVertexExecutor) executeStreamWithAPIKey(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options, apiKey, baseURL string) (_ *voltgateexecutor.StreamResult, err error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
...
func (e *GeminiVertexExecutor) countTokensWithServiceAccount(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options, projectID, location string, saJSON []byte) (voltgateexecutor.Response, error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
	...
	if err != nil {
		return voltgateexecutor.Response{}, err
	}
	...
	if errNewReq != nil {
		return voltgateexecutor.Response{}, errNewReq
	}
	httpReq.Header.Set("Content-Type", "application/json")
	if token, errTok := vertexAccessToken(ctx, e.cfg, auth, saJSON); errTok == nil && token != "" {
		httpReq.Header.Set("Authorization", "Bearer "+token)
		...
		log.Errorf("vertex executor: access token error: %v", errTok)
		return voltgateexecutor.Response{}, statusErr{code: 500, msg: "internal server error"}
	}
	applyGeminiHeaders(httpReq, auth)
	...
	if errDo != nil {
		recordAPIResponseError(ctx, e.cfg, errDo)
		return voltgateexecutor.Response{}, errDo
	}
	defer func() {
		if errClose := httpResp.Body.Close(); errClose != nil {
			log.Errorf("vertex executor: close response body error: %v", errClose)
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		appendAPIResponseChunk(ctx, e.cfg, b)
		logWithRequestID(ctx).Debugf("request error, error status: %d, error message: %s", httpResp.StatusCode, summarizeErrorBody(httpResp.Header.Get("Content-Type"), b))
		return voltgateexecutor.Response{}, statusErr{code: httpResp.StatusCode, msg: string(b)}
	}
	data, errRead := io.ReadAll(httpResp.Body)
	if errRead != nil {
		recordAPIResponseError(ctx, e.cfg, errRead)
		return voltgateexecutor.Response{}, errRead
	}
	appendAPIResponseChunk(ctx, e.cfg, data)
	count := gjson.GetBytes(data, "totalTokens").Int()
	out := sdktranslator.TranslateTokenCount(ctx, to, from, count, data)
	return voltgateexecutor.Response{Payload: []byte(out), Headers: httpResp.Header.Clone()}, nil
}

// countTokensWithAPIKey handles token counting using API key credentials.
func (e *GeminiVertexExecutor) countTokensWithAPIKey(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options, apiKey, baseURL string) (voltgateexecutor.Response, error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
...
func vertexCreds(a *voltgateauth.Auth) (projectID, location string, serviceAccountJSON []byte, err error) {
...
func vertexAPICreds(a *voltgateauth.Auth) (apiKey, baseURL string) {
	if a == nil {
		return "", ""
	}
	if a.Attributes != nil {
...
func vertexAccessToken(ctx context.Context, cfg *config.Config, auth *voltgateauth.Auth, saJSON []byte) (string, error) {
...
func (e *GeminiVertexExecutor) resolveVertexConfig(auth *voltgateauth.Auth) *config.VertexCompatKey {
