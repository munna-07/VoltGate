...
import (
	...
	"github.com/voltgate/voltgate/v6/internal/wsrelay"
...
func (e *AIStudioExecutor) PrepareRequest(_ *http.Request, _ *voltgateauth.Auth) error {
	return nil
}

// HttpRequest forwards an arbitrary HTTP request through the websocket relay.
func (e *AIStudioExecutor) HttpRequest(ctx context.Context, auth *voltgateauth.Auth, req *http.Request) (*http.Response, error) {
	if req == nil {
...
func (e *AIStudioExecutor) Execute(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (resp voltgateexecutor.Response, err error) {
	...
	resp = voltgateexecutor.Response{Payload: ensureColonSpacedJSON([]byte(out)), Headers: wsResp.Headers.Clone()}
	return resp, nil
}

// ExecuteStream performs a streaming request to the AI Studio API.
func (e *AIStudioExecutor) ExecuteStream(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (_ *voltgateexecutor.StreamResult, err error) {
	if opts.Alt == "responses/compact" {
	...
	go func(first wsrelay.StreamEvent) {
		...
		processEvent := func(event wsrelay.StreamEvent) bool {
			if event.Err != nil {
				...
				out <- voltgateexecutor.StreamChunk{Err: fmt.Errorf("wsrelay: %v", event.Err)}
				return false
			}
			switch event.Type {
			...
			case wsrelay.MessageTypeStreamChunk:
				if len(event.Payload) > 0 {
					...
					for i := range lines {
						out <- voltgateexecutor.StreamChunk{Payload: ensureColonSpacedJSON([]byte(lines[i]))}
					}
					break
				}
	...
	return &voltgateexecutor.StreamResult{Headers: firstEvent.Headers.Clone(), Chunks: out}, nil
}

// CountTokens counts tokens for the given request using the AI Studio API.
func (e *AIStudioExecutor) CountTokens(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
	...
	if resp.Status < 200 || resp.Status >= 300 {
		return voltgateexecutor.Response{}, statusErr{code: resp.Status, msg: string(resp.Body)}
	}
	totalTokens := gjson.GetBytes(resp.Body, "totalTokens").Int()
	if totalTokens <= 0 {
		return voltgateexecutor.Response{}, fmt.Errorf("wsrelay: totalTokens missing in response")
	}
...
func (e *AIStudioExecutor) Refresh(_ context.Context, auth *voltgateauth.Auth) (*voltgateauth.Auth, error) {
	return auth, nil
}
...
func (e *AIStudioExecutor) translateRequest(req voltgateexecutor.Request, opts voltgateexecutor.Options, stream bool) ([]byte, translatedPayload, error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
