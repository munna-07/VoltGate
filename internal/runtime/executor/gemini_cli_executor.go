...
import (
	...
	geminiauth "github.com/voltgate/voltgate/v6/internal/auth/gemini"
	"github.com/voltgate/voltgate/v6/internal/config"
	"github.com/voltgate/voltgate/v6/internal/misc"
	"github.com/voltgate/voltgate/v6/internal/runtime/geminicli"
...
func (e *GeminiCLIExecutor) PrepareRequest(req *http.Request, auth *voltgateauth.Auth) error {
	if req == nil {
		return nil
	}
...
func (e *GeminiCLIExecutor) HttpRequest(ctx context.Context, auth *voltgateauth.Auth, req *http.Request) (*http.Response, error) {
	if req == nil {
...
func (e *GeminiCLIExecutor) Execute(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (resp voltgateexecutor.Response, err error) {
...
func (e *GeminiCLIExecutor) ExecuteStream(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (_ *voltgateexecutor.StreamResult, err error) {
	if opts.Alt == "responses/compact" {
	...
	for idx, attemptModel := range models {
		...
		go func(resp *http.Response, reqBody []byte, attemptModel string) {
				...
				for scanner.Scan() {
					...
					if bytes.HasPrefix(line, dataTag) {
						...
						for i := range segments {
							out <- voltgateexecutor.StreamChunk{Payload: []byte(segments[i])}
						}
					}
				}
			...
			if errRead != nil {
				recordAPIResponseError(ctx, e.cfg, errRead)
				reporter.publishFailure(ctx)
				out <- voltgateexecutor.StreamChunk{Err: errRead}
				return
			}
			appendAPIResponseChunk(ctx, e.cfg, data)
			reporter.publish(ctx, parseGeminiCLIUsage(data))
...
func (e *GeminiCLIExecutor) CountTokens(ctx context.Context, auth *voltgateauth.Auth, req voltgateexecutor.Request, opts voltgateexecutor.Options) (voltgateexecutor.Response, error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
	...
	for range models {
		...
		tok, errTok := tokenSource.Token()
		if errTok != nil {
			return voltgateexecutor.Response{}, errTok
		}
		updateGeminiCLITokenMetadata(auth, baseTokenData, tok)
		...
		if resp.StatusCode >= 200 && resp.StatusCode < 300 {
			...
			return voltgateexecutor.Response{Payload: []byte(translated), Headers: resp.Header.Clone()}, nil
		}
	...
	return voltgateexecutor.Response{}, newGeminiStatusErr(lastStatus, lastBody)
}

// Refresh refreshes the authentication credentials (no-op for Gemini CLI).
func (e *GeminiCLIExecutor) Refresh(_ context.Context, auth *voltgateauth.Auth) (*voltgateauth.Auth, error) {
	return auth, nil
}

func prepareGeminiCLITokenSource(ctx context.Context, cfg *config.Config, auth *voltgateauth.Auth) (oauth2.TokenSource, map[string]any, error) {
	...
	clientID := stringValue(base, "client_id")
	if clientID == "" {
...
func updateGeminiCLITokenMetadata(auth *voltgateauth.Auth, base map[string]any, tok *oauth2.Token) {
	if auth == nil || tok == nil {
		return
	}
...
func resolveGeminiProjectID(auth *voltgateauth.Auth) string {
	if auth == nil {
		return ""
	}
...
func geminiOAuthMetadata(auth *voltgateauth.Auth) map[string]any {
	if auth == nil {
		return nil
	}
...
func newHTTPClient(ctx context.Context, cfg *config.Config, auth *voltgateauth.Auth, timeout time.Duration) *http.Client {
