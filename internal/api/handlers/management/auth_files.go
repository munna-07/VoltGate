...
import (
	...
	"github.com/voltgate/voltgate/v6/internal/auth/antigravity"
	"github.com/voltgate/voltgate/v6/internal/auth/claude"
	"github.com/voltgate/voltgate/v6/internal/auth/codex"
	...
	"github.com/voltgate/voltgate/v6/internal/auth/kimi"
	"github.com/voltgate/voltgate/v6/internal/auth/qwen"
	...
	for _, e := range entries {
		...
		if info, errInfo := e.Info(); errInfo == nil {
			...
			if data, errRead := os.ReadFile(full); errRead == nil {
				...
				projectValue := strings.TrimSpace(gjson.GetBytes(data, "project_id").String())
				expiresValue := strings.TrimSpace(gjson.GetBytes(data, "expired").String())
				fileData["type"] = typeValue
				fileData["email"] = emailValue
				if projectValue != "" {
					fileData["project_id"] = projectValue
				}
				if expiresValue != "" {
					fileData["expires_at"] = expiresValue
				}
...
func (h *Handler) buildAuthFileEntry(auth *coreauth.Auth) gin.H {
	...
	if hasQuotaState(auth.Quota) {
		entry["quota"] = auth.Quota
	}
	if len(auth.ModelStates) > 0 {
		entry["model_states"] = cloneModelStates(auth.ModelStates)
	}
	if path != "" {
		entry["path"] = path
		entry["source"] = "file"
	...
	modelsCount, modelPreview := authModelsSummary(auth.ID)
	if modelsCount > 0 {
		entry["models_count"] = modelsCount
		entry["models_preview"] = modelPreview
	}
...
func hasQuotaState(state coreauth.QuotaState) bool {
	return state.Exceeded || strings.TrimSpace(state.Reason) != "" || !state.NextRecoverAt.IsZero() || state.BackoffLevel > 0
}

func cloneModelStates(states map[string]*coreauth.ModelState) map[string]*coreauth.ModelState {
	if len(states) == 0 {
		return nil
	}
...
func authModelsSummary(authID string) (int, []string) {
	...
	preview := make([]string, 0, len(models))
	...
	for _, model := range models {
		...
		seen[modelID] = struct{}{}
		preview = append(preview, modelID)
	}
	sort.Strings(preview)
	if len(preview) > 6 {
		preview = preview[:6]
	}
	return len(seen), preview
}
...
func (h *Handler) RequestGeminiCLIToken(c *gin.Context) {
	...
	if !geminiAuth.CredentialsConfigured() {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": geminiAuth.MissingCredentialsError().Error()})
		return
	}
...
func (h *Handler) RequestAntigravityToken(c *gin.Context) {
	...
	if !antigravity.CredentialsConfigured() {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": antigravity.MissingCredentialsError().Error()})
		return
	}
