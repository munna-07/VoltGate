// Package api provides the HTTP API server implementation for the VoltGate.
...
import (
	...
	"github.com/voltgate/voltgate/v6/internal/access"
	managementHandlers "github.com/voltgate/voltgate/v6/internal/api/handlers/management"
	"github.com/voltgate/voltgate/v6/internal/api/middleware"
	"github.com/voltgate/voltgate/v6/internal/api/modules"
	ampmodule "github.com/voltgate/voltgate/v6/internal/api/modules/amp"
	"github.com/voltgate/voltgate/v6/internal/config"
	"github.com/voltgate/voltgate/v6/internal/logging"
	"github.com/voltgate/voltgate/v6/internal/managementasset"
	"github.com/voltgate/voltgate/v6/internal/usage"
	"github.com/voltgate/voltgate/v6/internal/util"
	sdkaccess "github.com/voltgate/voltgate/v6/sdk/access"
	"github.com/voltgate/voltgate/v6/sdk/api/handlers"
	"github.com/voltgate/voltgate/v6/sdk/api/handlers/claude"
	"github.com/voltgate/voltgate/v6/sdk/api/handlers/gemini"
	"github.com/voltgate/voltgate/v6/sdk/api/handlers/openai"
	sdkAuth "github.com/voltgate/voltgate/v6/sdk/auth"
	"github.com/voltgate/voltgate/v6/sdk/voltgate/auth"
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	if !cfg.RemoteManagement.AllowRemote && strings.TrimSpace(cfg.RemoteManagement.SecretKey) == "" && isLoopbackManagementHost(cfg.Host) {
		envAdminPassword = ""
		envAdminPasswordSet = false
	}
	...
	// Register management routes when secrets are available, or when the server is operating
	// in local-only management mode so localhost users can manage the runtime without a password.
	hasManagementSecret := cfg.RemoteManagement.SecretKey != "" || envManagementSecret || s.localPassword != ""
	localOnlyManagement := !cfg.RemoteManagement.AllowRemote
	s.managementRoutesEnabled.Store(hasManagementSecret || localOnlyManagement)
	if hasManagementSecret || localOnlyManagement {
...
func (s *Server) setupRoutes() {
	...
	s.engine.GET("/accounts.html", s.serveAccountCenter)
...
func (s *Server) serveAccountCenter(c *gin.Context) {
	cfg := s.cfg
	if cfg == nil || cfg.RemoteManagement.DisableControlPanel {
		c.AbortWithStatus(http.StatusNotFound)
		return
	...
	content := strings.TrimSpace(managementasset.AccountCenterHTML())
	if content == "" {
		c.AbortWithStatus(http.StatusNotFound)
		return
	}
...
func corsMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.Header("Access-Control-Expose-Headers", "X-CPA-Selected-Auth-ID")
...
func (s *Server) UpdateClients(cfg *config.Config) {
	...
	if s.envManagementSecret {
		...
		case !prevSecretEmpty && newSecretEmpty:
			if localOnlyManagement {
				s.registerManagementRoutes()
				s.managementRoutesEnabled.Store(true)
			} else if s.managementRoutesEnabled.CompareAndSwap(true, false) {
				log.Info("management routes disabled after secret key removal")
			} else {
				s.managementRoutesEnabled.Store(false)
			}
		default:
			s.managementRoutesEnabled.Store(!newSecretEmpty || localOnlyManagement)
		}
	}
