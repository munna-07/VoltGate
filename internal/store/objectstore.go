...
func (s *ObjectTokenStore) Save(ctx context.Context, auth *voltgateauth.Auth) (string, error) {
	if auth == nil {
...
func (s *ObjectTokenStore) List(_ context.Context) ([]*voltgateauth.Auth, error) {
	dir := strings.TrimSpace(s.AuthDir())
	if dir == "" {
		return nil, fmt.Errorf("object store: auth directory not configured")
	}
	entries := make([]*voltgateauth.Auth, 0, 32)
...
func (s *ObjectTokenStore) resolveAuthPath(auth *voltgateauth.Auth) (string, error) {
	if auth == nil {
		return "", fmt.Errorf("object store: auth is nil")
	}
	if auth.Attributes != nil {
...
func (s *ObjectTokenStore) readAuthFile(path, baseDir string) (*voltgateauth.Auth, error) {
	data, err := os.ReadFile(path)
	if err != nil {
	...
	auth := &voltgateauth.Auth{
		...
		Status:           voltgateauth.StatusActive,
		Attributes:       attr,
		Metadata:         metadata,
