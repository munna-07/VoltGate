...
func (s *GitTokenStore) Save(_ context.Context, auth *voltgateauth.Auth) (string, error) {
	if auth == nil {
...
func (s *GitTokenStore) List(_ context.Context) ([]*voltgateauth.Auth, error) {
	if err := s.EnsureRepository(); err != nil {
		return nil, err
	}
...
func (s *GitTokenStore) readAuthFile(path, baseDir string) (*voltgateauth.Auth, error) {
...
func (s *GitTokenStore) resolveAuthPath(auth *voltgateauth.Auth) (string, error) {
	if auth == nil {
		return "", fmt.Errorf("auth filestore: auth is nil")
	}
	if auth.Attributes != nil {
