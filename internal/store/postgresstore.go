...
func (s *PostgresStore) Save(ctx context.Context, auth *voltgateauth.Auth) (string, error) {
	if auth == nil {
...
func (s *PostgresStore) List(ctx context.Context) ([]*voltgateauth.Auth, error) {
	...
	auths := make([]*voltgateauth.Auth, 0, 32)
	for rows.Next() {
		var (
			id        string
			payload   string
...
func (s *PostgresStore) resolveAuthPath(auth *voltgateauth.Auth) (string, error) {
	if auth == nil {
		return "", fmt.Errorf("postgres store: auth is nil")
	}
	if auth.Attributes != nil {
