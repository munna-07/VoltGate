...
Add the module with `go get github.com/voltgate/voltgate/v6/sdk/access`.
...
In the CLI server and `sdk/voltgate`, this provider is registered automatically based on the loaded configuration.
...
The blank identifier import ensures `init` runs so `sdkaccess.RegisterProvider` executes before you call `RegisteredProviders()` (or before `voltgate.NewBuilder().Build()`).
...
## Integration with voltgate Service

`sdk/voltgate` wires `@sdk/access` automatically when you build a CLI service via `voltgate.NewBuilder`. Supplying a manager lets you reuse the same instance in your host process:
...
```go
// configaccess is github.com/voltgate/voltgate/v6/internal/access/config_access
