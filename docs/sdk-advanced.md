...
svc, _ := voltgate.NewBuilder().WithConfig(cfg).WithConfigPath(cfgPath).WithCoreAuthManager(core).Build()
```
...
```go
models := []*voltgate.ModelInfo{
  { ID: "myprov-pro-1", Object: "model", Type: "myprov", DisplayName: "MyProv Pro 1" },
}
voltgate.GlobalModelRegistry().RegisterClient(authID, "myprov", models)
```
