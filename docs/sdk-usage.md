...
```bash
go get github.com/voltgate/voltgate/v6/sdk/voltgate
```
...
import (
    ...
    "github.com/voltgate/voltgate/v6/sdk/voltgate"
)
```
...
```go
svc, _ := voltgate.NewBuilder().
  WithConfig(cfg).
  WithConfigPath("config.yaml").
  WithServerOptions(
    // Add global middleware
    voltgate.WithMiddleware(func(c *gin.Context) { c.Header("X-Embed", "1"); c.Next() }),
    // Tweak gin engine early (CORS, trusted proxies, etc.)
    voltgate.WithEngineConfigurator(func(e *gin.Engine) { e.ForwardedByClientIP = true }),
    // Add your own routes after defaults
    voltgate.WithRouterConfigurator(func(e *gin.Engine, _ *handlers.BaseAPIHandler, _ *config.Config) {
      e.GET("/healthz", func(c *gin.Context) { c.String(200, "ok") })
    }),
    // Override request log writer/dir
    voltgate.WithRequestLoggerFactory(func(cfg *config.Config, cfgPath string) logging.RequestLogger {
...
func (p *memoryTokenProvider) Load(ctx context.Context, cfg *config.Config) (*voltgate.TokenClientResult, error) {
    // Populate from memory/remote store and return counts
    return &voltgate.TokenClientResult{}, nil
}
...
svc, _ := voltgate.NewBuilder().
  ...
  WithAPIKeyClientProvider(voltgate.NewAPIKeyClientProvider()).
  Build()
```
...
```go
hooks := voltgate.Hooks{
  OnBeforeStart: func(cfg *config.Config) { log.Infof("starting on :%d", cfg.Port) },
  OnAfterStart:  func(s *voltgate.Service) { log.Info("ready") },
}
svc, _ := voltgate.NewBuilder().WithConfig(cfg).WithConfigPath("config.yaml").WithHooks(hooks).Build()
```
