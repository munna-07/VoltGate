...
// and integrate it with the VoltGate server. This example shows how to:
...
import (
	...
	"github.com/voltgate/voltgate/v6/sdk/api"
	...
	clipexec "github.com/voltgate/voltgate/v6/sdk/voltgate/executor"
	"github.com/voltgate/voltgate/v6/sdk/config"
	"github.com/voltgate/voltgate/v6/sdk/logging"
	sdktr "github.com/voltgate/voltgate/v6/sdk/translator"
)
...
func main() {
	...
	hooks := voltgate.Hooks{
		OnAfterStart: func(s *voltgate.Service) {
			// Register demo models for the custom provider so they appear in /v1/models.
			models := []*voltgate.ModelInfo{{ID: "myprov-pro-1", Object: "model", Type: providerKey, DisplayName: "MyProv Pro 1"}}
			for _, a := range core.List() {
				if strings.EqualFold(a.Provider, providerKey) {
					voltgate.GlobalModelRegistry().RegisterClient(a.ID, providerKey, models)
				}
			}
		},
	}
